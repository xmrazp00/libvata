...
int main()
{
    ...
    //Loading Init automaton - on back run it's Bad states automaton
    Automaton AutAlpha;
    AutAlpha.LoadFromString(*parser, Bad1, stateDictInit);
    const VATA::ExplicitTreeAut autInit = AutAlpha;

    //Loading Bad States Automaton (in back run case it's Init automaton)
    Automaton BadStates;

    BadStates.LoadFromString(*parser, Init, stateDictBad);
